...
// Helper to wrap promises with a timeout to avoid 10s connection freezes
...
export async function seedDatabaseIfEmpty() {
  try {
    const disasterSnap = await withTimeout(getDocs(collection(db, 'disasters')), 1000);
    if (disasterSnap.empty) {
      ...
      for (const item of INITIAL_DISASTERS) {
        await withTimeout(setDoc(doc(db, 'disasters', item.id), item), 1000).catch(() => {});
      }
      // Seed Shelters
      for (const item of INITIAL_SHELTERS) {
        await withTimeout(setDoc(doc(db, 'shelters', item.id), item), 1000).catch(() => {});
      }
      // Seed Resources
      for (const item of INITIAL_RESOURCES) {
        await withTimeout(setDoc(doc(db, 'resources', item.id), item), 1000).catch(() => {});
      }
      // Seed Requests
      for (const item of INITIAL_REQUESTS) {
        await withTimeout(setDoc(doc(db, 'requests', item.id), item), 1000).catch(() => {});
      }
      // Seed NGOs
      for (const item of INITIAL_NGOS) {
        await withTimeout(setDoc(doc(db, 'ngos', item.id), item), 1000).catch(() => {});
      }
      // Seed Volunteers
      for (const item of INITIAL_VOLUNTEERS) {
        await withTimeout(setDoc(doc(db, 'volunteers', item.id), item), 1000).catch(() => {});
      }
      // Seed Deliveries
      for (const item of INITIAL_DELIVERIES) {
        await withTimeout(setDoc(doc(db, 'deliveries', item.id), item), 1000).catch(() => {});
      }
      // Seed Notifications
      for (const item of INITIAL_NOTIFICATIONS) {
        await withTimeout(setDoc(doc(db, 'notifications', item.id), item), 1000).catch(() => {});
      }
      // Seed Activity Logs
      for (const item of INITIAL_ACTIVITY_LOGS) {
        await withTimeout(setDoc(doc(db, 'activity_logs', item.id), item), 1000).catch(() => {});
      }
