...
export const Settings: React.FC = () => {
  ...
  const [profileOrg, setProfileOrg] = useState(currentUser?.organization || '');
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!profileName.trim()) {
      setProfileError('Display Name is a required field.');
      return;
    }
    ...
    try {
      ...
      await logSecurityAction('User Profile Update', `Updated user profile settings: Name: ${profileName}, Org: ${profileOrg}`);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-7 space-y-6">
        ...
        <div id="user-profile-settings-panel" className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          <h3 className="text-sm font-bold text-slate-900 uppercase font-mono tracking-wider flex items-center space-x-1.5 border-b border-slate-100 pb-3">
            <UserIcon className="h-4.5 w-4.5 text-rose-600" />
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-4 mt-5">
            {profileSuccess && (
              <div className="p-3 bg-emerald-50 text-emerald-800 border border-emerald-200 rounded-lg text-xs font-semibold flex items-center space-x-1.5 animate-in fade-in duration-200">
                ...
                <span>Profile synchronization successful. Credentials updated securely.</span>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-450 uppercase font-mono mb-1.5">Full Name</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-450 uppercase font-mono mb-1.5">Primary Email Address</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled
                    className="w-full pl-9 pr-3 py-2 text-xs border border-slate-200 rounded-lg bg-slate-50 text-slate-400 cursor-not-allowed font-mono"
                  />
                </div>
                <span className="text-[9px] text-slate-400 mt-1 block">Account identifiers are immutable.</span>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-450 uppercase font-mono mb-1.5">Contact Number</label>
                <div className="relative">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-450 uppercase font-mono mb-1.5">Organization / Agency</label>
                <div className="relative">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-100 mt-4">
              <div className="text-[10px] text-slate-500 font-mono">
                Assigned Role: <span className="font-bold text-rose-600">{currentUser?.role}</span>
              </div>
              <button
                type="submit"
                disabled={profileSaving}
                className="bg-rose-600 hover:bg-rose-700 disabled:opacity-70 text-white text-xs font-semibold px-4 py-2 rounded-lg flex items-center space-x-1.5 cursor-pointer shadow-sm shadow-rose-900/10 transition-colors"
              >
                {profileSaving ? (
